...
def process_seg(img_file, img_mask, clothes_mask, input_res=512):
    ...
    img_mask = crop(img_mask, center, scale, (input_res, input_res))

    clothes_mask = torch.from_numpy(clothes_mask)
    clothes_mask = crop(clothes_mask, center, scale, (input_res, input_res))
    clothes_mask = torch.from_numpy(clothes_mask).unsqueeze(0)
...
def process_image(img_file, det, hps_type, input_res=512,seg_path=None):
    ...
    if img_mask is not None:
        img_mask = torch.from_numpy(img_mask)[:,:,0]
        img_mask[img_mask>0] = 1 # should already be ones but making sure
        bbox = masks_to_boxes(img_mask.unsqueeze(0))[0]
        width = bbox[2] - bbox[0]
        height = bbox[3] - bbox[1]
        center = np.array([(bbox[0] + bbox[2]) / 2.0,
                           (bbox[1] + bbox[3]) / 2.0])
    ...
    with torch.no_grad():
        ...
        img_pil = Image.open(
            io.BytesIO((buf.getvalue()))).convert("RGBA")
